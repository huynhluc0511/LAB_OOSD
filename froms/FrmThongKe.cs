        ...
        private void ThongKeData()
        {
            try
            {
                ...
                string sqlPhieuDat = $@"SELECT COUNT(*) FROM ""PhieuDatPhong"" 
                                      WHERE ""NgayLap""::date BETWEEN '{tuNgay}' AND '{denNgay}'";
                DataTable dtPhieuDat = Db.Query(sqlPhieuDat);
                ...
                string sqlDangO = @"SELECT COUNT(*) FROM ""PhieuDatPhong"" WHERE ""TrangThai"" = 'Đang ở'";
                DataTable dtDangO = Db.Query(sqlDangO);
                int countDangO = (dtDangO != null && dtDangO.Rows.Count > 0) ? Convert.ToInt32(dtDangO.Rows[0][0]) : 0;
                lblDangO.Text = $"Đang ở: {countDangO}";

                // 3. Thống kê Hóa đơn và Doanh thu
                string sqlHoaDon = $@"SELECT COUNT(*), COALESCE(SUM(""TongTien""), 0) FROM ""HoaDon"" 
                                      WHERE ""NgayLap""::date BETWEEN '{tuNgay}' AND '{denNgay}'";
                DataTable dtHoaDon = Db.Query(sqlHoaDon);
                int countHoaDon = 0;
                decimal doanhThu = 0;
                ...
                try
                {
                    string sqlDenBu = $@"SELECT COALESCE(SUM(""TongTien""), 0) FROM ""PhieuDenBu"" 
                                        WHERE ""NgayLap""::date BETWEEN '{tuNgay}' AND '{denNgay}'";
                    DataTable dtDenBu = Db.Query(sqlDenBu);
                    if (dtDenBu != null && dtDenBu.Rows.Count > 0)
                    {
                        tongDenBu = Convert.ToDecimal(dtDenBu.Rows[0][0]);
                    }
                }
                catch
                {
                ...
                // 5. Thống kê Chi tiết Dịch vụ
                string sqlDichVu = $@"SELECT 
                                        dv.""MaDV"" AS ""Mã DV"", 
                                        dv.""TenDV"" AS ""Tên dịch vụ"", 
                                        COALESCE(SUM(ct.""SoLuong""), 0) AS ""Tổng số lượng"", 
                                        COALESCE(SUM(ct.""ThanhTien""), 0) AS ""Tổng tiền""
                                      FROM ""DichVu"" dv
                                      LEFT JOIN ""ChiTietPhieuSuDungDV"" ct ON dv.""MaDV"" = ct.""MaDV""
                                      LEFT JOIN ""PhieuSuDungDV"" p ON ct.""SoPhieuSDDV"" = p.""SoPhieuSDDV"" 
                                           AND p.""NgaySuDung""::date BETWEEN '{tuNgay}' AND '{denNgay}'
            ...
            {
                MessageBox.Show("Lỗi thống kê: " + ex.Message, "Thông báo lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
}